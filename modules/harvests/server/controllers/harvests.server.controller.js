 ...
 * List of Harvests
 */
exports.list = function(req, res) { Harvest.find().sort('-created').populate('user', 'displayName').exec(function(err, harvests) {
		if (err) {
			return res.send(400, {
				message: getErrorMessage(err)
			});
 ...
 * Harvest middleware
 */
exports.harvestByID = function(req, res, next, id) { Harvest.findById(id).populate('user', 'displayName').exec(function(err, harvest) {
