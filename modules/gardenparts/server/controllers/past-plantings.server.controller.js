...
var addPastPlantings = function(next,req){
    ...
    Planting.find({ $and: [
            ...
            //Is planting valid?
            {'validTo':{$lte: req.params.selectedDate}},
            {validFrom: {$gte: minSixYears}},
            { $or: [
                //Is rightTopCorner in gardenpart?
                {$and: [{'elemleft' : { $gte: req.gardenpart.elemleft}}, {'elemleft': {$lt: rightCornerLeft }},{'elemtop' : { $gte: req.gardenpart.elemtop}}, {'elemtop': {$lt: bottomCornerTop }}]},
                //Is leftTopCorner in gardenpart?
                {$and: [{'rightCornerLeft' : { $gte: req.gardenpart.elemleft}}, {'rightCornerLeft': {$lt: rightCornerLeft }},{'elemtop' : { $gte: req.gardenpart.elemtop}}, {'elemtop': {$lt: bottomCornerTop }}]},
                //Is leftBottomCorner in gardenpart?
                {$and: [{'elemleft' : { $gte: req.gardenpart.elemleft}}, {'elemleft': {$lt: rightCornerLeft }},{'bottomCornerTop' : { $gte: req.gardenpart.elemtop}}, {'bottomCornerTop': {$lt: bottomCornerTop }}]},
                //Is rightBottomCorner in gardenpart?
                {$and: [{'rightCornerLeft' : { $gte: req.gardenpart.elemleft}}, {'rightCornerLeft': {$lt: rightCornerLeft }},{'bottomCornerTop' : { $gte: req.gardenpart.elemtop}}, {'bottomCornerTop': {$lt: bottomCornerTop }}]}
            ]}
        ]},
...
exports.gardenpartByBK = function(req, res, next, gardenpartbk) {
    Gardenpart.findOne({validFrom: {$lte: req.params.selectedDate},validTo: {$gt: req.params.selectedDate}, bk: gardenpartbk}).populate('user', 'displayName').sort('-validFrom').lean().exec(function(err, gardenpart) {
        if (err) return next(err);
