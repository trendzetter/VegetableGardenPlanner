...
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("friends",{abstract:!0,url:"/friends",template:"<ui-view/>"}).state("friends.list",{url:"",templateUrl:"/modules/friends/client/views/list-friends.client.view.html",controller:"FriendsListController",controllerAs:"vm",data:{pageTitle:"Friends List"}}).state("friends.search",{url:"/search",templateUrl:"/modules/friends/client/views/search-friends.client.view.html",controller:"FriendsSearchController",controllerAs:"vm",data:{pageTitle:"Friends Search"}}).state("friends.view",{url:"/:friendId",templateUrl:"/modules/friends/client/views/view-friend.client.view.html",controller:"FriendsController",controllerAs:"vm",resolve:{friendResolve:getFriend},data:{pageTitle:"Friend {{ friendResolve.title }}"}})}function getFriend($stateParams,FriendsService){return FriendsService.get({friendId:$stateParams.friendId}).$promise}angular.module("friends.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getFriend.$inject=["$stateParams","FriendsService"]}();
...
!function(){"use strict";function FriendsSearchController(FriendsService){function search(){FriendsService.search({friendName:vm.friendname}).then(function(users){vm.users=users})}var vm=this;vm.search=search}angular.module("friends").controller("FriendsSearchController",FriendsSearchController),FriendsSearchController.$inject=["FriendsService"]}();
!function(){"use strict";function FriendsService($resource,$log){function createOrUpdate(friend){function onSuccess(friend){}function onError(errorResponse){var error=errorResponse.data;handleError(error)}return friend._id?friend.$update(onSuccess,onError):friend.$save(onSuccess,onError)}function handleError(error){$log.error(error)}var Friend=$resource("/api/friends/:friendId",{friendId:"@_id"},{update:{method:"PUT"},search:{method:"GET",url:"/api/friends/search/:friendName",isArray:!0}});return angular.extend(Friend.prototype,{createOrUpdate:function(){var friend=this;return createOrUpdate(friend)}}),Friend}angular.module("friends.services").factory("FriendsService",FriendsService),FriendsService.$inject=["$resource","$log"]}();
