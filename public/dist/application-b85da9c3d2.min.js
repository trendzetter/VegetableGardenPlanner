!function(window){"use strict";function registerModule(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)}var applicationModuleName="mean",service={applicationEnvironment:window.env,applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:["ngResource","ngAnimate","ngMessages","ui.router","ui.bootstrap","ui.bootstrap.contextMenu","angularFileUpload","dragon-drop","ngCropper","ngFileUpload","ngImgCrop","ngCookies","pascalprecht.translate"],registerModule:registerModule};window.ApplicationConfiguration=service}(window);
!function(app){"use strict";function bootstrapConfig($compileProvider,$locationProvider,$httpProvider,$translateProvider){$translateProvider.useStaticFilesLoader({prefix:"i18n/",suffix:".json"}),$translateProvider.useLocalStorage(),$translateProvider.preferredLanguage("en"),$translateProvider.useSanitizeValueStrategy("escape"),$locationProvider.html5Mode(!0).hashPrefix("!"),$httpProvider.interceptors.push("authInterceptor"),$compileProvider.debugInfoEnabled("production"!==app.applicationEnvironment)}function init(){if(window.location.hash&&"#_=_"===window.location.hash)if(window.history&&history.pushState)window.history.pushState("",document.title,window.location.pathname);else{var scroll={top:document.body.scrollTop,left:document.body.scrollLeft};window.location.hash="",document.body.scrollTop=scroll.top,document.body.scrollLeft=scroll.left}angular.bootstrap(document,[app.applicationModuleName])}angular.module(app.applicationModuleName,app.applicationModuleVendorDependencies),angular.module(app.applicationModuleName).config(bootstrapConfig),bootstrapConfig.$inject=["$compileProvider","$locationProvider","$httpProvider","$translateProvider"],angular.element(document).ready(init)}(ApplicationConfiguration);
...
!function(app){"use strict";app.registerModule("notifications",["core"]),app.registerModule("notifications.admin",["core.admin"]),app.registerModule("notifications.admin.routes",["core.admin.routes"]),app.registerModule("notifications.services"),app.registerModule("notifications.routes",["ui.router","core.routes","notifications.services"])}(ApplicationConfiguration);
...
!function(){"use strict";function routeConfig($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise(function($injector,$location){$injector.get("$state").transitionTo("not-found",null,{location:!1})}),$stateProvider.state("home",{url:"/",templateUrl:"modules/core/client/views/home.client.view.html",controller:"HomeController",controllerAs:"vm"}).state("not-found",{url:"/not-found",templateUrl:"modules/core/client/views/404.client.view.html",data:{ignoreState:!0,pageTitle:"Not-Found"}}).state("bad-request",{url:"/bad-request",templateUrl:"modules/core/client/views/400.client.view.html",data:{ignoreState:!0,pageTitle:"Bad-Request"}}).state("forbidden",{url:"/forbidden",templateUrl:"modules/core/client/views/403.client.view.html",data:{ignoreState:!0,pageTitle:"Forbidden"}})}angular.module("core.routes").config(routeConfig),routeConfig.$inject=["$stateProvider","$urlRouterProvider"]}();
...
!function(){"use strict";function menuConfig(Menus){Menus.addMenuItem("topbar",{title:"system",state:"system",type:"dropdown",roles:["user","admin"]}),Menus.addSubMenuItem("topbar","system",{title:"list_crops",state:"crops.list"}),Menus.addSubMenuItem("topbar","system",{title:"add_crop",state:"crops.create",roles:["user"]})}angular.module("crops").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("topbar","system",{title:"list_cultivation_plans",state:"cultivation-plans.list"}),menuService.addSubMenuItem("topbar","system",{title:"add_cultivation_plan",state:"cultivation-plans.create",roles:["user"]})}angular.module("cultivation-plans").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function menuConfig(Menus){Menus.addSubMenuItem("topbar","admin",{title:"Manage Notifications",state:"admin.notifications.list"})}angular.module("notifications.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.notifications.list",{url:"",templateUrl:"modules/notifications/client/views/admin/list-notifications.client.view.html",controller:"NotificationsAdminListController",controllerAs:"vm",data:{roles:["admin"]}}).state("admin.notifications.create",{url:"/create",templateUrl:"modules/notifications/client/views/admin/form-notification.client.view.html",controller:"NotificationsAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{notificationResolve:newNotification}}).state("admin.notifications.edit",{url:"/:notificationId/edit",templateUrl:"modules/notifications/client/views/admin/form-notification.client.view.html",controller:"NotificationsAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{notificationResolve:getNotification}})}function getNotification($stateParams,NotificationsService){return NotificationsService.get({notificationId:$stateParams.notificationId}).$promise}function newNotification(NotificationsService){return new NotificationsService}angular.module("notifications.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getNotification.$inject=["$stateParams","NotificationsService"],newNotification.$inject=["NotificationsService"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("topbar",{title:"notifications",state:"notifications",type:"dropdown",roles:["user"]}),menuService.addSubMenuItem("topbar","notifications",{title:"list_notifications",state:"notifications.list",roles:["user"]})}angular.module("notifications").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("notifications",{"abstract":!0,url:"/notifications",template:"<ui-view/>"}).state("notifications.list",{url:"",templateUrl:"modules/notifications/client/views/list-notifications.client.view.html",controller:"NotificationsListController",controllerAs:"vm",data:{pageTitle:"Notifications List"}}).state("notifications.view",{url:"/:notificationId",templateUrl:"modules/notifications/client/views/view-notification.client.view.html",controller:"NotificationsController",controllerAs:"vm",resolve:{notificationResolve:getNotification},data:{pageTitle:"Notification {{ notificationResolve.title }}"}})}function getNotification($stateParams,NotificationsService){return NotificationsService.get({notificationId:$stateParams.notificationId}).$promise}angular.module("notifications.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getNotification.$inject=["$stateParams","NotificationsService"]}();
!function(){"use strict";function NotificationsListController(NotificationsService){var vm=this;vm.notifications=NotificationsService.query()}angular.module("notifications").controller("NotificationsListController",NotificationsListController),NotificationsListController.$inject=["NotificationsService"]}();
!function(){"use strict";function NotificationsController($scope,notification,Authentication){var vm=this;vm.notification=notification,vm.authentication=Authentication,vm.error=null}angular.module("notifications").controller("NotificationsController",NotificationsController),NotificationsController.$inject=["$scope","notificationResolve","Authentication"]}();
!function(){"use strict";function NotificationsService($resource){function createOrUpdate(notification){function onSuccess(notification){}function onError(errorResponse){var error=errorResponse.data;handleError(error)}return notification._id?notification.$update(onSuccess,onError):notification.$save(onSuccess,onError)}function handleError(error){console.log(error)}var Notification=$resource("api/notifications/:notificationId",{notificationId:"@_id"},{update:{method:"PUT"}});return angular.extend(Notification.prototype,{createOrUpdate:function(){var notification=this;return createOrUpdate(notification)}}),Notification}angular.module("notifications.services").factory("NotificationsService",NotificationsService),NotificationsService.$inject=["$resource"]}();
!function(){"use strict";function menuConfig(Menus){Menus.addSubMenuItem("topbar","system",{title:"list_plant_families",state:"plantfamilies.list"}),Menus.addSubMenuItem("topbar","system",{title:"add_plant_family",state:"plantfamilies.create",roles:["user"]})}angular.module("plantfamilies").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function PlantFamiliesController($scope,$state,$window,plantfamily,Authentication){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.plantfamily.$remove($state.go("plantfamilies.list"))}function save(isValid){function successCallback(res){$state.go("plantfamilies.view",{plantFamilyId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.plantfamily._id?vm.plantfamily.$update(successCallback,errorCallback):vm.plantfamily.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.plantFamilyForm"),!1)}var vm=this;vm.plantfamily=plantfamily,vm.authentication=Authentication,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("plantfamilies").controller("PlantFamiliesController",PlantFamiliesController),PlantFamiliesController.$inject=["$scope","$state","$window","plantFamilyResolve","Authentication"]}();
...
!function(){"use strict";function PlantVarietiesController($scope,$state,$timeout,$window,plantvariety,Authentication,CropsService,FileUploader,Cropper){function showCropper(){$scope.$broadcast($scope.showEvent)}function remove(){$window.confirm("Are you sure you want to delete?")&&vm.plantvariety.$remove($state.go("plant-varieties.list"))}function save(isValid){function successCallback(res){$state.go("plant-varieties.view",{plantVarietyId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.plantvariety._id?vm.plantvariety.$update(successCallback,errorCallback):vm.plantvariety.$save(successCallback,errorCallback)):(console.log("save!"),$scope.$broadcast("show-errors-check-validity","vm.form.plantvarietyForm"),!1)}var vm=this;if(vm.plantvariety=plantvariety,vm.authentication=Authentication,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.crops=CropsService.query(),"plant-varieties.view"===$state.current.name){var year=(new Date).getFullYear(),dateStart=new Date(year,0);vm.DOYstartSow=new Date(dateStart.setDate(plantvariety.DOYstartSow));var dateEnd=new Date(year,0);vm.DOYendSow=new Date(dateEnd.setDate(plantvariety.DOYendSow));var today=new Date;vm.DOYendSow<today?(console.log("Zaaien is al voorbij vm.DOYendSow: "+vm.DOYendSow+"< today: "+today),vm.selectedDate=vm.DOYstartSow.getFullYear()+1+"-"+("0"+(vm.DOYstartSow.getMonth()+1)).substr(-2)+"-"+("0"+vm.DOYstartSow.getDate()).substr(-2)):vm.DOYstartSow<today?(console.log("Zaaien is al begonnen neem vandaag"),vm.selectedDate=today.getFullYear()+"-"+("0"+(today.getMonth()+1)).substr(-2)+"-"+("0"+today.getDate()).substr(-2)):(console.log(" vm.DOYendSow: "+vm.DOYendSow+">= today: "+today),console.log(" vm.DOYstartSow: "+vm.DOYstartSow+">= today: "+today),console.log("Zaaien is al nog niet begonnen neem dit jaar zaaiperiode start"),vm.selectedDate=vm.DOYstartSow.getFullYear()+"-"+("0"+(vm.DOYstartSow.getMonth()+1)).substr(-2)+"-"+("0"+vm.DOYstartSow.getDate()).substr(-2))}var file,data;$scope.options={maximize:!0,aspectRatio:1,crop:function(dataNew){data=dataNew,$scope.scale(128)}},$scope.preview=function(){file&&data&&Cropper.crop(file,data).then(Cropper.encode).then(function(dataUrl){(vm.plantvariety||(vm.plantvariety={})).image=dataUrl})},$scope.scale=function(width){Cropper.crop(file,data).then(function(blob){return Cropper.scale(blob,{width:width})}).then(Cropper.encode).then(function(dataUrl){(vm.plantvariety||(vm.plantvariety={})).image=dataUrl})},$scope.showEvent="show",$scope.hideEvent="hide",$scope.uploader=new FileUploader,$scope.uploader.onAfterAddingFile=function(fileItem){Cropper.encode(file=fileItem._file).then(function(dataUrl){$scope.dataUrl=dataUrl,$timeout(showCropper)})}}angular.module("plant-varieties").controller("PlantVarietiesController",PlantVarietiesController),PlantVarietiesController.$inject=["$scope","$state","$timeout","$window","plantVarietyResolve","Authentication","CropsService","FileUploader","Cropper"]}();
...
!function(){"use strict";function PlantVarietiesService($resource){return $resource("api/plant-varieties/:plantVarietyId",{plantVarietyId:"@_id"},{update:{method:"PUT"},getCrop:{method:"GET",url:"api/plant-varieties/get-crop/:cropId",isArray:!0}})}angular.module("plant-varieties.services").factory("PlantVarietiesService",PlantVarietiesService),PlantVarietiesService.$inject=["$resource"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("topbar","system",{title:"list_rule_sets",state:"rulesets.list"}),menuService.addSubMenuItem("topbar","system",{title:"create_rule_set",state:"rulesets.create",roles:["user"]})}angular.module("rule-sets").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function NotificationsAdminListController(NotificationsService){var vm=this;vm.notifications=NotificationsService.query()}angular.module("notifications.admin").controller("NotificationsAdminListController",NotificationsAdminListController),NotificationsAdminListController.$inject=["NotificationsService"]}();
!function(){"use strict";function NotificationsAdminController($scope,$state,$window,notification,Authentication){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.notification.$remove(function(){$state.go("admin.notifications.list")})}function save(isValid){function successCallback(res){$state.go("admin.notifications.list")}function errorCallback(res){vm.error=res.data.message}return isValid?void vm.notification.createOrUpdate().then(successCallback)["catch"](errorCallback):($scope.$broadcast("show-errors-check-validity","vm.form.notificationForm"),!1)}var vm=this;vm.notification=notification,vm.authentication=Authentication,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("notifications.admin").controller("NotificationsAdminController",NotificationsAdminController),NotificationsAdminController.$inject=["$scope","$state","$window","notificationResolve","Authentication"]}();
...
!function(){"use strict";function UserController($scope,$state,$window,Authentication,user){function remove(user){$window.confirm("Are you sure you want to delete this user?")&&(user?(user.$remove(),vm.users.splice(vm.users.indexOf(user),1)):vm.user.$remove(function(){$state.go("admin.users")}))}function update(isValid){if(!isValid)return $scope.$broadcast("show-errors-check-validity","vm.userForm"),!1;var user=vm.user;user.$update(function(){$state.go("admin.user",{userId:user._id})},function(errorResponse){vm.error=errorResponse.data.message})}function isContextUserSelf(){return vm.user.username===vm.authentication.user.username}var vm=this;vm.authentication=Authentication,vm.user=user,vm.remove=remove,vm.update=update,vm.isContextUserSelf=isContextUserSelf}angular.module("users.admin").controller("UserController",UserController),UserController.$inject=["$scope","$state","$window","Authentication","userResolve"]}();
!function(){"use strict";function ChangePasswordController($scope,$http,Authentication,UsersService,PasswordValidator){function changeUserPassword(isValid){return vm.success=vm.error=null,isValid?void UsersService.changePassword(vm.passwordDetails).then(onChangePasswordSuccess)["catch"](onChangePasswordError):($scope.$broadcast("show-errors-check-validity","vm.passwordForm"),!1)}function onChangePasswordSuccess(response){$scope.$broadcast("show-errors-reset","vm.passwordForm"),vm.success=!0,vm.passwordDetails=null}function onChangePasswordError(response){vm.error=response.data.message}var vm=this;vm.user=Authentication.user,vm.changeUserPassword=changeUserPassword,vm.getPopoverMsg=PasswordValidator.getPopoverMsg}angular.module("users").controller("ChangePasswordController",ChangePasswordController),ChangePasswordController.$inject=["$scope","$http","Authentication","UsersService","PasswordValidator"]}();
