...
!function(){"use strict";function GardensListController(GardensService,$stateParams){var vm=this;if($stateParams.plant)vm.selectedDate=$stateParams.selectedDate,vm.plant=$stateParams.plant,vm.gardens=GardensService.getPlantGardens();else{var today=new Date;vm.selectedDate=today.getFullYear()+"-"+("0"+(today.getMonth()+1)).substr(-2)+"-"+("0"+today.getDate()).substr(-2),vm.gardens=GardensService.query()}}angular.module("gardens").controller("GardensListController",GardensListController),GardensListController.$inject=["GardensService","$stateParams"]}();
...
!function(){"use strict";function PlantGardenController($scope,$state,garden,Authentication,$stateParams,GardenpartsService,RuleSetsService){function cropgroupByCrop(crop){var cropgroup=!1;loop:for(var c=0;c<garden.ruleset.cropgroups.length;c++)for(var crops=garden.ruleset.cropgroups[c].crops,d=0;d<crops.length;d++)if(crops[d]._id===crop){cropgroup=garden.ruleset.cropgroups[c]._id;break loop}return cropgroup}function successCallback(res){var crop=garden.plantVariety.crop,cropgroup=cropgroupByCrop(crop);console.log("calculating the advice! crop: "+crop+" cropgroup: "+cropgroup);for(var pp=garden.pastplantings,j=0;j<pp.length;j++){var planting=pp[j];planting.elemtop=parseInt(planting.elemtop)-vm.garden.elemtop,planting.elemleft=parseInt(planting.elemleft)-vm.garden.elemleft,planting.cropgroup=cropgroupByCrop(planting.plantVariety.crop),planting.greenlevel=null,planting.redlevel=null;var yearsApart=new Date(new Date(vm.selectedDate)-new Date(planting.validTo)).getFullYear()-1970;if(console.log("years apart: "+yearsApart+" date:"+planting.validTo),planting.opacity=1-yearsApart*yearsApart/36,2>yearsApart&&cropgroup===planting.cropgroup)planting.greenlevel=0,planting.redlevel=255;else for(var rotationrules=vm.garden.ruleset.rotationrules,k=0;k<rotationrules.length;k++)console.log("cropgroup"+cropgroup+"rotationrules[k].cropgroup"+rotationrules[k].cropgroup),console.log("previousCropgroup:"+rotationrules[k].previousCropgroup+" planting.cropgroup: "+planting.cropgroup),cropgroup===rotationrules[k].cropgroup&&rotationrules[k].previousCropgroup===planting.cropgroup&&(console.log("rule applies, checking time"),rotationrules[k].yearsBetween>yearsApart?(planting.greenlevel=0,planting.redlevel=255):(planting.greenlevel=255,planting.redlevel=0))}}function errorCallback(res){vm.error=res.data.message}var vm=this;vm.garden=garden,vm.authentication=Authentication,vm.selectedDate=$stateParams.selectedDate,vm.error=null,vm.form={},$scope.setDate=function(date){vm.selectedDate=date},vm.garden.ruleset=RuleSetsService.get({ruleSetId:garden.ruleset},successCallback,errorCallback)}angular.module("gardens").controller("PlantGardenController",PlantGardenController),PlantGardenController.$inject=["$scope","$state","gardenResolve","Authentication","$stateParams","GardenpartsService","RuleSetsService"]}();
...
"use strict";angular.module("gardens").directive("calendar",["$state",function($state){return{require:"ngModel",link:function(scope,el,attr,ngModel){el.datepicker({changeMonth:!0,changeYear:!0,dateFormat:"yy-mm-dd",onSelect:function(dateText){scope.$apply(function(){ngModel.$setViewValue(dateText),scope.setDate(dateText);var mode=attr.calendar;("view"===mode||"edit"===mode)&&$state.go("viewGarden",{bk:scope.vm.garden.bk,selectedDate:dateText})})}})}}}]);
