...
"use strict";angular.module("gardenparts").directive("zoomable",["$rootScope",function($rootScope){return{restrict:"A",link:function(scope,element,attrs){var broadcastZoom=function(){scope.$broadcast("updatedZoom"),$rootScope.$broadcast("updatedZoom",scope.zoom)};scope.$on("plantingsLoaded",function(){console.log("plantingsLoaded in plantingpart"),scope.zoom=2,broadcastZoom()}),scope.zoomIn=function(){scope.zoom+=.2,broadcastZoom()},scope.zoomOut=function(){scope.zoom-=.2,broadcastZoom()}}}}]);
!function(){"use strict";function GardenpartService($resource,$stateParams){return $resource("api/gardenpart/:bk/:selectedDate",{bk:"@bk",selectedDate:$stateParams.selectedDate},{update:{method:"PUT"}})}angular.module("gardenparts.services").factory("GardenpartService",GardenpartService),GardenpartService.$inject=["$resource","$stateParams"]}();
...
"use strict";angular.module("gardens").config(["$stateProvider",function($stateProvider){$stateProvider.state("listGardens",{url:"/gardens/list/:plant/:gardendate",templateUrl:"modules/gardens/client/views/list-gardens.client.view.html",controller:"GardensListController",controllerAs:"vm",data:{pageTitle:"Gardens list"}}).state("listGardenversions",{url:"/gardenversions",templateUrl:"modules/gardens/client/views/list-garden-versions.client.view.html"}).state("createGarden",{url:"/gardens/create",templateUrl:"modules/gardens/client/views/create-garden.client.view.html"}).state("viewGarden",{url:"/gardens/:bk/:selectedDate",templateUrl:"modules/gardens/client/views/view-garden.client.view.html"}).state("plantGarden",{url:"/gardens/plant/:bk/:selectedDate/:plant",templateUrl:"modules/gardens/client/views/view-garden.client.view.html"}).state("viewGardenversion",{url:"/gardenversion/:gardenId/:selectedDate",templateUrl:"modules/gardens/client/views/view-gardenversion.client.view.html"}).state("editGarden",{url:"/gardens/:gardenId/edit/:selectedDate",templateUrl:"modules/gardens/client/views/edit-garden.client.view.html"}).state("designGarden",{url:"/gardens/:gardenId/layout/:selectedDate",templateUrl:"modules/gardens/client/views/layout-garden.client.view.html"})}]);
...
"use strict";angular.module("gardens").controller("FormController",["$scope","Gardendata","Authentication","$rootScope",function($scope,Gardendata,Authentication,$rootScope){if($scope.create=function(){$rootScope.$broadcast("createGarden")},$scope.updateGarden=function(){Gardendata.setGarden($scope.garden)},$scope.$on("error",function(){$scope.error=Gardendata.getError()}),$scope.authentication=Authentication,$scope.garden=Gardendata.getGarden(),$scope.setDate=function(date){$scope.garden.selectedDate=date},"undefined"===$scope.garden.selectedDate){var today=new Date;$scope.garden.selectedDate=today.getFullYear()+"-"+("0"+(today.getMonth()+1)).substr(-2)+"-"+("0"+today.getDate()).substr(-2),console.log("selected date! "+$scope.garden.selectedDate)}}]);
...
"use strict";angular.module("gardens").directive("calendar",[function(){return{require:"ngModel",link:function(scope,el,attr,ngModel){console.log("calendar "),el.datepicker({dateFormat:"yy-mm-dd",onSelect:function(dateText){scope.$apply(function(){ngModel.$setViewValue(dateText),scope.setDate(dateText),console.log("attr: "+attr.calendar);var mode=attr.calendar;("view"===mode||"edit"===mode)&&scope.findOne()})}})}}}]);
"use strict";angular.module("gardens").directive("dragtool",[function(){var draggableConfig={};return{restrict:"A",link:function(scope,elem,attrs){var css={"z-index":"1040",left:window.innerWidth/2,width:"240px"};elem.css(css),elem.draggable(draggableConfig),elem.addClass("col-md-"+attrs.dragtool+" panel panel-default dragtool"),elem.prepend('            <div class="panel-heading glyphicon glyphicon-move" style="width: 100%"/>')}}}]);
"use strict";angular.module("gardens").directive("garden",[function(){var resizableConfig={grid:[5,5],handles:"all",containment:"parent"},draggableConfig={animate:!0,grid:[5,5],containment:"parent"};return{restrict:"A",link:function(scope,elem,attrs){var mode=attrs.garden;"create"===mode?(elem.addClass("creategarden"),elem.draggable(draggableConfig),scope.updateCoordinates(50,50,250,350),elem.on("drag",function(evt,ui){scope.updateCoordinates(ui.position.top,ui.position.left,elem.height(),elem.width()),scope.$apply()})):scope.$on("gardenpartsLoaded",function(){var topmin=Number.MAX_VALUE,offsettop=Number.MIN_VALUE,leftmin=Number.MAX_VALUE,offsetleft=Number.MIN_VALUE;angular.forEach(scope.gardenparts,function(gardenpart,key){topmin>gardenpart.elemtop&&(topmin=gardenpart.elemtop),leftmin>gardenpart.elemleft&&(leftmin=gardenpart.elemleft);var partoffsettop=gardenpart.elemtop+gardenpart.elemheight;partoffsettop>offsettop&&(offsettop=partoffsettop);var partoffsetleft=gardenpart.elemleft+gardenpart.elemwidth;partoffsetleft>offsetleft&&(offsetleft=partoffsetleft)}),scope.minHeight=offsettop-topmin,scope.minWidth=offsetleft-leftmin,scope.initialMinHeight=offsettop-topmin,scope.initialMinWidth=offsetleft-leftmin,scope.initialTop=scope.garden.elemtop,scope.initialLeft=scope.garden.elemleft,scope.initialHeight=scope.garden.elemheight,scope.initialWidth=scope.garden.elemwidth,elem.resizable("option","minHeight",scope.minHeight),elem.resizable("option","minWidth",scope.minWidth)}),elem.resizable(resizableConfig),elem.on("resize",function(evt,ui){scope.updateCoordinates(ui.position.top,ui.position.left,ui.size.height,ui.size.width);"create"!==mode&&(elem.resizable("option","minHeight",scope.minHeight),elem.resizable("option","minWidth",scope.minWidth),ui.size.height=scope.garden.elemheight,ui.position.top=scope.garden.elemtop,ui.size.width=scope.garden.elemwidth,ui.position.left=scope.garden.elemleft),scope.$apply()})}}}]);
"use strict";var getCoordinates=function(elem){var el=$(elem),co={};return co.top=el.css("top").replace("px",""),co.left=el.css("left").replace("px",""),co.height=el.css("height").replace("px",""),co.width=el.css("width").replace("px",""),co};angular.module("gardens").directive("gardenpart",["$timeout",function($timeout){return{restrict:"A",link:function(scope,elem,attrs){scope.popoverEvent=function(){$timeout(function(){elem.trigger("customEvent")},0)};var updateCoordinates=function(){var co=getCoordinates(elem);scope.update(co.top,co.left,co.width,co.height),scope.$apply()},resizeStop=function(){scope.gardenpart.modified=!0,console.log("modified:"+JSON.stringify(scope.gardenpart)),updateCoordinates()};if(elem.addClass(scope.gardenpart.type),"layout"===attrs.gardenpart){var resizableConfig={containment:"#garden",autoHide:!1,snap:!0,snapmode:"outer",stop:resizeStop,resize:updateCoordinates};switch(scope.gardenpart.type){case"hweg":resizableConfig.minHeight=25,resizableConfig.maxHeight=60,resizableConfig.handles="n,s,e,w";break;case"vweg":resizableConfig.minWidth=25,resizableConfig.maxWidth=60,resizableConfig.handles="n,s,e,w";break;case"akke":resizableConfig.minHeight=50,resizableConfig.minWidth=50,resizableConfig.handles="n,s,e,w";break;case"vbed":resizableConfig.minWidth=60,resizableConfig.maxWidth=120,resizableConfig.handles="n,s,e,w";break;case"hbed":resizableConfig.minHeight=60,resizableConfig.maxHeight=120,resizableConfig.handles="n,s,w,e";break;default:console.log("you shouldnt get here default case in switch gardenpart directive"+JSON.stringify(scope.gardenpart))}if(elem.resizable(resizableConfig),elem.droppable(),"undefined"==typeof scope.gardenpart.bk){switch(scope.gardenpart.type){case"hweg":scope.update(0,0,150,30);break;case"vweg":scope.update(0,0,30,150);break;case"akke":scope.update(0,0,50,50);break;case"vbed":scope.update(0,0,100,200);break;case"hbed":scope.update(0,0,200,100);break;default:console.log("you shouldnt get here default case in switch gardenpart directive"+JSON.stringify(scope.gardenpart))}var draggableConfig={animate:!0,grid:[1,1],containment:"parent",revert:"valid",snap:".gardenpart ",snapmode:"outer"};draggableConfig.drag=updateCoordinates,elem.draggable(draggableConfig)}}}}}]);
...
!function(){"use strict";function GardensService($resource,$stateParams){return $resource("api/gardens/:bk/:selectedDate",{bk:"@bk",selectedDate:$stateParams.selectedDate},{update:{method:"PUT"}})}angular.module("gardens.services").factory("GardensService",GardensService),GardensService.$inject=["$resource","$stateParams"]}();
