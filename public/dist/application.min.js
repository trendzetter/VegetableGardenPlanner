...
!function(){"use strict";function CultivationPlansController($scope,$state,cultivationPlan,$window,Authentication,Crops){function newStep(){var step={};vm.cultivationPlan.steps.push(step)}function removeStep(index){console.log("remove step:"+index),vm.cultivationPlan.steps.splice(index,1)}function remove(){$window.confirm("Are you sure you want to delete?")&&vm.cultivationPlan.$remove($state.go("cultivation-plans.list"))}function save(isValid){function successCallback(res){$state.go("cultivation-plans.view",{cultivationPlanId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.cultivationPlan._id?vm.cultivationPlan.$update(successCallback,errorCallback):vm.cultivationPlan.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.cultivationPlanForm"),!1)}var vm=this;vm.cultivationPlan=cultivationPlan,vm.authentication=Authentication,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.crops=Crops.query(),vm.crops.$promise.then(function(crops){cultivationPlan._id||(console.log("new cultivationPlan"),vm.cultivationPlan.steps=[],newStep())}),vm.newStep=newStep,vm.removeStep=removeStep}angular.module("cultivation-plans").controller("CultivationPlansController",CultivationPlansController),CultivationPlansController.$inject=["$scope","$state","cultivationPlanResolve","$window","Authentication","CropsService"]}();
...
!function(){"use strict";function CultivationPlansService($resource){return $resource("api/cultivation-plans/:articleId",{articleId:"@_id"},{update:{method:"PUT"}})}angular.module("cultivation-plans.services").factory("CultivationPlansService",CultivationPlansService),CultivationPlansService.$inject=["$resource"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("topbar","system",{title:"Manage Users",state:"admin.users"})}angular.module("users.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
